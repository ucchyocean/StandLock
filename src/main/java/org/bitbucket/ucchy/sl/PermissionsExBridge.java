...
public class PermissionsExBridge {
    ...
    /**
     * 指定されたプレイヤーのオプション値をintegerで取得する
     * @param player プレイヤー
     * @param option オプション名
     * @param defaultValue オプションのデフォルト値
     * @return オプションの値
     */
    public int getPlayerIntegerOptionValue(Player player, String option, int defaultValue) {
        PermissionUser user = PermissionsEx.getPermissionManager().getUser(player);
        if ( user != null ) {
            return user.getOptionInteger(option, player.getWorld().getName(), defaultValue);
        }
        return defaultValue;
    }
}
