 ...
 * @author ucchy
 */
public class StandLockCommand implements TabExecutor {

    protected static final String META_INFO_COMMAND = "standlockinfo";
    protected static final String META_PRIVATE_COMMAND = "standlockprivate";
     ...
     * @param parent
     */
    public StandLockCommand(StandLock parent) {
        this.parent = parent;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        // 各種短縮コマンド
        if ( command.getName().equals("sinfo") ) {
            return doInfo(sender, command, label, args);
        } else if ( command.getName().equals("slimits") ) {
            return doLimits(sender, command, label, args);
        } else if ( command.getName().equals("sprivate") ) {
            return doPrivate(sender, command, label, args);
        } else if ( command.getName().equals("sremove") ) {
            return doRemove(sender, command, label, args);
        } else if ( command.getName().equals("spersist") ) {
            return doPersist(sender, command, label, args);
        }

        // 以下、/standlock コマンドに対する処理
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if ( command.getName().equals("standlock") ) {
            if ( args.length == 1 ) {
